...
function buildWhatsAppShareUrl(location) {
  ...
  if (location.address && location.address.trim()) lines.push(location.address.trim());
  lines.push(mapsUrl);
...
async function fetchRouteSummary(points) {
  ...
  if (missing.length) {
    return { error: `These locations have no saved coordinates, so distance can't be calculated: ${missing.map((p) => p.name).join(', ')}. Add "lat,lng" for them in Manage Locations, or remove them from the route.` };
  }
...
function LocationPicker({ locations, valueId, onSelect, placeholder }) {
  ...
  const selected = valueId ? locations.find((l) => String(l.id) === String(valueId)) : null;
  ...
  if (!editing) {
    return (
      <button type="button" className="locationPickerClosed" onClick={() => setEditing(true)}>
        <span className={selected ? 'locationPickerValue' : 'locationPickerPlaceholder'}>
          {selected ? selected.name : (placeholder || 'Select location')}
        </span>
        <span className="locationPickerRightIcons">
          {selected && <span className="locationPickerClear" onClick={clear} role="button" aria-label="Clear selection">✕</span>}
          <span className="locationPickerCaret">▾</span>
        </span>
      </button>
    );
  }
...
function QuickShareLocations({ locations }) {
  ...
  return (
    <section className="card">
      <div className="sectionTitle">
        <span className="iconCircle iconGold">💬</span>
        <div><h2>Quick Share</h2><p>Find a location and send it via WhatsApp instantly</p></div>
      </div>
      <div className="dateField">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search a location by name..." />
      </div>
      <div className="quickShareList">
        ...
        {results.length > 8 && <p className="loadingText">{results.length - 8} more match — keep typing to narrow it down.</p>}
      </div>
    </section>
  );
}
      ...
      <section className="card">
        ...
        {!loading && locations.length > 0 && (
          <>
            ...
            {summaryMessage && <div className="errorMessage">⚠ {summaryMessage}</div>}
              ...
              <div className="routeSummaryBox fadeIn">
                {summary.legs.map((leg, i) => (
                  <div className="routeSummaryLeg" key={i}>
                    ...
                    <span className="routeSummaryLegStats">{leg.km.toFixed(1)} km · {Math.round(leg.min)} min</span>
                  </div>
                ))}
...
function LocationManager({ onBack }) {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error, needsSortOrderColumn } = await fetchLocationsData();
    ...
    if (!needsSortOrderColumn && data.some((l) => l.sort_order === null || l.sort_order === undefined)) {
      ...
      const refetch = await fetchLocationsData();
    ...
    <>
      ...
      {needsMigration && (
        <div className="errorMessage">
          ⚠ Ordering isn't set up yet — run this once in the Supabase SQL editor, then reload this page:
          {' '}<code>alter table locations add column sort_order integer;</code>
        </div>
      )}
